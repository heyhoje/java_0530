...
public class _2_MapEx {
	...
	public static void main(String[] args) {
		...
		map2.put("zxc", "abc");
		System.out.println(map2); // 중복된 값을 넣으면 덮어쓰기됨
		
		/* keySet을 이용한 반복문 - key값들을 Set으로 만들어서 반복 */
		// ㄴHashSet을 하려다 에러떠서 Set으로 바꿈
		// 이거 자체가 뭐하려는 앤지 모르겠는데요... 
		Set<String> keySet = map2.keySet();
		Iterator<String> it = keySet.iterator(); // iterator가 뭐시냐...
		while(it.hasNext()){ // hasNext()는 뭐지
			String id = it.next(); // key값을 가져옴
			String pw = map2.get(id); // 번지가 아니라, key값(id)을 이용해서 value값을 가져옴
