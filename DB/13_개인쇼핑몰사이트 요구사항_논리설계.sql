...
/*
- 내가 파악한 요구사항,,,,
  - 소비자는 회원가입을 통해 프로그램을 이용한다.
  - 소비자는 회원가입시, 이름, 아이디, 주소, 전화번호 등을 입력한다.
  - 판매상품은 관리자가 등록한다.
  - 판매상품은 상품번호, 상품명, 가격, 재고를 입력한다.
  - 관리자가 판매상품을 상품 등록 한다.
  - 상품을 등록을 위해 ()를 입력한다.
  - 소비자는 등록된 판매상품을 구매한다.
*/
...
관계를 논리적으로 설계할 때는, 경우에 따라 다음과 같이 작성한다
...
회원(아이디(PK), 비번, 전화번호, 주소, 상세주소, 우편번호, 마일리지, 권한, 이름, +아이디(FK))
배송지(배송지번호(PK), 배송지명, 주소, 상세주소, 우편번호, 회원아이디(FK)) -- 1:n 보유관계일경우 n쪽에 외래키를 적용 
...
- 속성이 있는 관계들은 새 릴레이션을 만든다.
+ 주문(주문번호(PK), 총금액, 결제금액, 사용 마일리지, 적립 마일리지, 상태, +아이디(FK), 배송지번호(FK))
