...
DDL (데이터 정의어) : 테이블/DB를 추가, 수정, 삭제 / 버튼으로 가능
DML (데이터 조작어) : 데이터 추가, 수정, 조회 (** 공부를 많이해야하는 부분)
DCL (데이터 제어어) : 계정에 사용권한을 부여, 회수 / 서버관리자가 해서 사용할 일이 적음 
TCL (트랜잭션 제어어) : 작업을 묶어서 DML 결과를 작업단위별로 묶어서 제어 /TCL을 하지 않아도 바로바로 작업이 됨. 신경쓰지 않아도 됨
*/
/*
DDL
- create   : 테이블/DB를 생성
...
*/
/*
DB 생성 - MySQL에서는 DB와 스키마가 같은 개념으로 인식됨 
create schema DB명;
create database DB명;
drop schema DB명;
drop database DB명;

if not exists DB명 : DB명이 존재하지 않을 때 작업됨
if exists DB명 : DB명이 존재할 때 작업됨
*/
...
create table 테이블명(
	속성명1 타입 [default 값] [not null | null] [auto-increment],
...
제약조건 옵션 : 
	- 참조무결성을 지키기 위해서 참조되는 테이블의 기본키값이 변경되면 
	참조하는 테이블의 값을 어떻게 할건지를 결정하는 값 
...
-- 수강 테이블 기본키에 있는 auto-imcrement 속성을 제거
...
alter table course drop constraint course_idfk_2;
