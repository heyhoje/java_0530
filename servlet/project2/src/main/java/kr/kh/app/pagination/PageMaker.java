...
public class PageMaker {
	...
	private int endPage;	// 페이지 네이션에서 마지막 페이지 번호
	...
	public void calculate() {
		// 주어지는 정보 : 현재 페이지 정보(criteria), 전체 게시글(endPage에 영향)
		// 			  한 페이지 네이션에서 보여주는 페이지 최대 개수(displayPageNum)
		// 예시 : 현재 페이지는 1페이지고, 한 페이지에 게시글이 5개, 전체 게시글은 15개, pagination 5개
		...
		startPage = (endPage - displayPageNum + 1); // 20 - 10 + 1 = 11로 시작
		...
		// (첫 페이지네이션이면 : ) 시작페이지가 1이면 이전버튼 비활성화 / 11, 21 이면 활성화
		prev = startPage == 1 ? false : true;
		// 마지막 페이지네이션이면) : lastEndPage와 endPage가 같으면 다음버튼 비활성화
